...
def init():
  global spi, display
  spi = initSPI()
  display = initOLED(spi)

def initSPI() -> SPI:
    return SPI(1,20000_000,polarity=0, phase=0,sck=Pin(SCK),mosi=Pin(MOSI),miso=None)

def initOLED(spi1: SPI) -> sh1107.SH1107_SPI:
    return sh1107.SH1107_SPI(
      width=WIDTH,
      height=HEIGHT,
      spi=spi1,
      dc=Pin(DC,Pin.OUT),
